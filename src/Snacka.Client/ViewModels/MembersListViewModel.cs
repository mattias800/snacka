...
/// Reads state from CommunityStore and VoiceStore (Redux-style).
/// </summary>
public class MembersListViewModel : ViewModelBase
{
    ...
    private async Task SaveNicknameAsync()
    {
        ...
        if (communityId is null || EditingNicknameMember is null) return;
        ...
        try
        {
            ...
            {
                result = await _apiClient.UpdateMyNicknameAsync(communityId.Value, nickname);
            }
            else
            {
                result = await _apiClient.UpdateMemberNicknameAsync(communityId.Value, EditingNicknameMember.UserId, nickname);
            }

            if (result.Success && result.Data is not null)
            {
                // Update store - collection will auto-update via DynamicData binding
                _communityStore.UpdateMemberNickname(communityId.Value, EditingNicknameMember.UserId, nickname);
                CancelNicknameEdit();
            }
            else
            {
    ...
    {
        var communityId = _communityStore.GetSelectedCommunityId();
        if (communityId is null || !CanManageMembers) return;
        ...
        try
        {
            var result = await _apiClient.UpdateMemberRoleAsync(communityId.Value, member.UserId, UserRole.Admin);
            if (result.Success && result.Data is not null)
            {
                // Update store - collection will auto-update via DynamicData binding
                _communityStore.UpdateMemberRole(communityId.Value, member.UserId, UserRole.Admin);
            }
            else
            {
                _onError(result.Error);
    ...
    private async Task DemoteToMemberAsync(CommunityMemberResponse member)
    {
        ...
        try
        {
            var result = await _apiClient.UpdateMemberRoleAsync(communityId.Value, member.UserId, UserRole.Member);
            if (result.Success && result.Data is not null)
            {
                // Update store - collection will auto-update via DynamicData binding
                _communityStore.UpdateMemberRole(communityId.Value, member.UserId, UserRole.Member);
            }
            else
            {
                _onError(result.Error);
    ...
    private async Task TransferOwnershipAsync(CommunityMemberResponse member)
    {
        ...
        try
        {
            var result = await _apiClient.TransferOwnershipAsync(communityId.Value, member.UserId);
            if (result.Success)
            {
                // Reload members to get updated roles
                var membersResult = await _apiClient.GetMembersAsync(communityId.Value);
                if (membersResult.Success && membersResult.Data is not null)
                {
                    // Update store - collection will auto-update via DynamicData binding
                    _communityStore.SetMembers(communityId.Value, membersResult.Data);
    ...
    private async Task AddStationToChannelAsync(MyGamingStationInfo station)
    {
        ...
        if (_voiceStore.GetCurrentChannelId() is null)
        {
